...
public sealed class ExamNavigationOrchestrator : IExamNavigationOrchestrator, IDisposable
{
    ...
    private void NavigateToCorrectPage(DateTime now)
    {
        ...
        bool isTimeShow = currentType == typeof(TimeWinUI.Views.TimeShowPage);
        bool isDashboard = currentType == typeof(TimeWinUI.Views.DashboardShowPage);

        if (shouldTime && !isTimeShow)
        {
            _navigation.NavigateTo(typeof(TimeWinUI.ViewModels.TimeShowViewModel).FullName!);
        }
        else if (!shouldTime && !isDashboard)
        {
            _navigation.NavigateTo(typeof(TimeWinUI.ViewModels.DashboardShowViewModel).FullName!);
        }
    }
